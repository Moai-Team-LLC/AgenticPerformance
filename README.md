...
- ✅ **Phase-0 код** (реестр) на ветке `feat/apl-phase0-registry`, tsc-green, не запушено.
- ✅ **GitHub issues** — приватный репо https://github.com/AlexDuchDev/apl, 37 issues из бэклога (см. `ISSUES.md`), метки `phase-0..5`/`post-mvp` + `MUST/SHOULD/MAY`.
- ⏳ Далее по плану: Phase-0 нормализация-контракт + fixture-продукты (APL-0.2/0.5 = #2/#5); затем Phase 1 (SDK + OTLP-ресивер + Timescale-стор).
